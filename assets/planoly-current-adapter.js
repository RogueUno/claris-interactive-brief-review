(() => {
  ...
  const buildCase = (result) => {
    ...
    const sources = rankedSources.map((source) => ({
      id: source.evidence_id,
      title: source.source_title,
      url: source.source_url,
      source_type: String(source.source_type || '').toLowerCase(),
      classification: source.source_type === 'FIRST_PARTY_PUBLIC' ? 'VERIFIED' : 'INFERRED',
      presentation_label: source.source_type === 'FIRST_PARTY_PUBLIC' ? 'Verified Source' : 'Inferred Signal'
    }));
    ...
    return {
      ...
      meeting: {
        ...
        timezone_abbr: consultant.display_timezone || 'UTC', meeting_url: null,
      ...
      evidence: {
        overall_research_coverage_pct: brief.user_facing_evidence?.coverage_score_pct ?? null,
        sources, curated_claims: claims, traceability_map: {},
        provenance_summary: `Current generic engine · live research snapshot · ${diagnostics.status || 'display synthesis'} · not qualified`
      }
    };
  };
