...
function render(brief){
  ...
  const ev=arr(prepare?.expandable_blocks?.evidence).map(e=>`<details id="${esc(e.id)}"><summary><b>${esc(e.id)}</b> ${esc(e.title)}</summary><div class="detailbody"><h5>Claims</h5>${arr(e.claims).map(x=>`<p>${esc(x)}</p>`).join('')}<h5>Sources</h5>${arr(e.sources).map(s=>`<p><a target="_blank" rel="noopener noreferrer" href="${safeUrl(s.url)}">${esc(s.url)}</a><br><small>${esc(s.supports)}</small></p>`).join('')}<h5>Used for</h5><p>${esc(arr(e.used_for).join(' · '))}</p><h5>Not used for</h5><p>${esc(arr(e.not_used_for).join(' · '))}</p></div></details>`).join('');
